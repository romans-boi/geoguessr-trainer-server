        ...
        get {
            ...
            val quizQuestions = quizRepository.generateQuiz(
                ...
                numOfQuestions = numOfQuestions.toIntOrNull()?.let { num ->
                    if (num < 5 || num > 50) {
                        return@get getInvalidError("$numOfQuestionsParam - must be 5 <= x <= 50")
                    } else {
                        num
                    }
                } ?: return@get getInvalidError(numOfQuestionsParam),

                numOfOptions = numOfOptions.toIntOrNull()?.let { num ->
                    if (num < 2 || num > 4) {
                        return@get getInvalidError("$numOfOptionsParam - must be 2 <= x <= 4")
                    } else {
                        num
                    }
                } ?: return@get getInvalidError(numOfOptionsParam)
            )
